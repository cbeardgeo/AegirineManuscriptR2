...
\subsection{Experimental equipment and procedures}
        ...
        ]{2_M5_Map_Partial-01.eps}
        \caption[Element maps of a clinopyroxene from internally heated pressure vessel experiment M5]{Element maps of clinopyroxene from internally heated pressure vessel experiment M5. Aegirine-augite clinopyroxene have a sharp internal boundary that reflects feldspar saturation and are subtly zoned. Further element maps are available as an electronic supplement (Fig. \ref{ElementMap_Supplement}).}
        \label{2_Elementmap}
        \end{center}
        \end{figure}
...
\subsection{Natural samples}
         ...
         \begin{figure}[ht]
        \begin{center}
        \includegraphics[width=0.80\textwidth]{3_CanariesTransects_Chem_BSE-01.eps}
...
Transects of Ce, Mg and Fe concentrations within the experiment clinopyroxene were measured with a JEOL 8900 instrument at McGill University using a focussed beam of 50 nA with an accelerating voltage of 20 kV. Ce was counted with a wavelength-dispersive spectrometer with an LIFH crystal for 100 s (MAC-Ce standard). Mg and Fe were counted for 20 s, with TAP and LIF crystals respectively (diopside and olivine standards, respectively).
%VvH v13: ADD STANDARDS HERE. 
Matrix corrections for Mg, Fe and Ce were not possible with these data, for lack of other major-elements measured, however their relative values may still be used to assess the extent to which the clinopyroxene are chemically zoned.  
...
Drift corrections and data reduction were conducted in Iolite v2.5 \citep{Paton2011}. The total of major-elements measured by LA-ICP-MS or, where available, the Al concentration from electron-microprobe analyses, was used as an internal standard (Table \ref{PxTbl}). For some experiments, ablation through the minerals was too rapid to generate a stable signal for data-reduction purposes. In these cases, a mixing model was applied to estimate the composition of these clinopyroxene, similar to that applied by \citet{Rubatto2007} (see supplementary methods \ref{LaserMix}). %and \citealt{Yang_InPrep}).
%ADD WHEN SUBMITTED
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Results} 
A total of 11 experiments are reported. Phase proportions, major-element and trace-element abundances in quenched melts and clinopyroxene are provided in Table \ref{Ae_SupTbl}; 25 additional experiments were rejected as their run temperatures were superliquidus or subsolidus, or their growth textures were indicative of disequilibrium (e.g. Fig. \ref{ElementMap_Supplement}). 

All reported experimental runs were near-liquidus ($<$22\% crystals)
...
, and are characterised by a homogeneous distribution of phases (Fig \ref{1_BSE}), except for experiments H$5_3$ and NLS-$9_{2-HM}$, in which sanidine crystals are concentrated at the centre. Glasses are clean and homogeneous, and clinopyroxene crystals are generally small, euhedral blades with a narrow range of sizes for a given experiment ($<$10 $\mu$m to 100 $\mu$m in cross section, Figs. \ref{1_BSE}--\ref{2_Elementmap}). 

Glasses are homogeneous and show a limited range of major- and trace-element compositions for each experiment (Fig. \ref{TAS}). In addition to clinopyroxene and glass, experiments on mafic to intermediate compositions produced magnetite, titanite $\pm$ kaersutite amphibole, whereas some phonolitic experiments produced biotite, alkali feldspar $\pm$ magnetite (Table \ref{Ae_SupTbl}). 

Low sums of major-element oxide concentrations in the electron microprobe analyses are a result of high \ce{Fe^{3+}} contents in the pyroxenes, and high water contents in the quenched melts (Supplement \ref{Ae_SupTbl}).
        ...
        {pyroxene grown in internally heated pressure vessel experiments (a) expressed in terms of  diopside, hedenbergite and aegirine end-members, (b) their M1Ti and tetrahedral Al and \ce{Fe^{3+}} contents, and (c) expressed in terms of aegirine, hedenbergite and the theoretical \ce{CaTiAl2O6} end-member \citep{Akasaka1980}. clinopyroxene compositions from Canary Islands volcanic rocks are shown for comparison (this study). 
        \ce{Fe^{2+}/Fe^{3+}} within the clinopyroxene was assigned following \citet{Droop1987}, then site occupancies and end-member proportions were allocated following \citet{Morimoto1989}. For full details on the end-member calculation procedure, see supplementary material.
        The range of clinopyroxene compositions within each experiment records `fractional crystallisation'
        %Fedele: I won't use such term (though I understand you wanted just to give an idea of the process), I think it is somewhat misleading. How about saying that "The range of clinopyroxene compositions within each experiment records changes in the compositon of the melt during crystal growth, whereby the cores of the crystals..."?
         during the course of the experiment, whereby the cores of crystals become chemically isolated from the melt. The rims (highest Fe, Na) of each experiment are in equilibrium with the adjacent melt.}
        \label{Px_Maj}
        \end{center}
        \end{figure}
...
The Canary Islands clinopyroxene are diopside\ce{_{60--90}}, with most crystals containing about 0.05 c.f.u. \ce{^{IV}Al} (in the T site) and 0.02 c.f.u. \ce{^{VI}Ti} (in the M1 site, Tables \ref{PxTbl}, \ref{Ae_SupTbl}, Fig. \ref{Px_Maj}). 
Experiments on the synthetic compositions produced clinopyroxene of diopside to aegirine-augite composition, a subset of which overlap with the compositional space defined by the Canary Islands on a diopside--hedenbergite--aegirine ternary diagram (Fig. \ref{Px_Maj}a). This low-aegirine group of synthetic clinopyroxene display a positive correlation between \ce{^{IV}Al} and \ce{^{VI}Ti} content (red, \emph{low aegirine}, \ce{Ae_{5--25}}), and notably contain both of these elements at elevated concentration relative to the Canary Islands clinopyroxene (\ce{^{IV}Al} = 0.20--0.45 c.f.u., \ce{^{VI}Ti} = 0.07--0.17 c.f.u.). A further subset of synthetic clinopyroxene have higher aegirine contents (blue, \emph{med aegirine}, \ce{Ae_{25--50}}) and show a similar range of Ti contents to the low aegirine synthetic minerals (0.05--0.18 c.f.u.), but a limited range of lower \ce{^{IV}Al} content (0.1--0.2 c.f.u.). Experiments performed on the Nechalacho layered suite composition (NLS-9, NLS-9\ce{_{2-HM}}, green, \emph{high aegirine}, \ce{Ae_{55--95}}) reproduced the aegirine-augite to aegirine clinopyroxene from the natural system and contain almost no \ce{^{IV}Al}, \ce{^{VI}Ti}, Ca or Mg \citep[][]{Moller2016}.

%% ADD differences here???
% Fedele: I do not agree, synthetic cpx crystals are definitely Ti-richer. According to table S1, only few Canary Island cpx have TiO2 > 3 wt.%, up to a maximum of about 4.5 wt.%, whereas most of the synthetic cpx have more than 3 wt.% TiO2, reaching values as high as 7.5 wt.%. Moreover, Canary cpx have very low Al2O3 (typically below 6 wt.%) whereas synthetic ones include numerous crystals with Al2O3 above 8 wt.%, up to 9-12 wt.%. In my opinion, this also speaks for the need for a much more detailed treatment of the comparison between the various datasets. In addition, it should be noted that Canary cpx are quite variable in composition, which in my opinion would make them unsuitable for the determination of trace element partition coefficients, unless some data filtering is made (i.e., in order to define which can be considered to have attained equilibrium with the melt and which not).


 The Canary Islands clinopyroxene that were selected for determination of mineral--melt partition coefficients are free from melt inclusions, suggesting relatively slow rates of crystal growth \citep[][Fig. \ref{3_ChemTransect}]{Kennedy1993}. Chemical zonation, if present, is more subtle than the analytical precision of our electron microprobe, indicating that pressure, temperature and melt composition remained stable during crystal growth. In the case of these Canary Islands clinopyroxene, the entire mineral is in chemical equilibrium with the adhered quenched melt. We are confident of this at a level greater than the analytical precision of electron microprobe analyses (typically $\sim$2\%). 
...
By definition, crystallisation changes the composition of the adjacent melt by preferentially removing compatible major-elements relative to those of lower compatibility, therefore all crystallisation induces some changes to melt composition.


This crystal-chemical zonation is systematic both within individual experiments and within groups of experiments and results from growth of crystals from a limited volume of melt within the experimental charges.


revealing three major-element exchange mechanisms. These exchange mechanisms correspond to low (\ce{Ae_{5--25}}), medium (\ce{Ae_{25--50}}) and high (\ce{Ae_{55--95}}) aegirine domains defined above (Figs. \ref{Px_Maj}b, \ref{ExchMechBar} and \ref{SF_ExchMech}). 
	With increasing alkali content, the Si content of clinopyroxene increases at the expense of tetrahedrally co-ordinated Al and \ce{Fe^{3+}}. As aegirine content increases, substitutions at the tetrahedral site become relatively less important than exchanges at the M1 and M2 sites.
...
Suites of Canary Islands clinopyroxene follow similar major-element exchange systematics to our low aegirine experiment clinopyroxenes (CHECK).



Clinopyroxene has the general formula M2M1\ce{T2O6}, with the M1 and M2 sites incorporating most of the trace-elements of geological interest \citep{Morimoto1989}. Iron in the clinopyroxene was assigned to 2+ or 3+ valence following \citet{Droop1987}, then major-element cations were assigned to sites following \citet[][see supplement \ref{EXCEL_D_Model}]{Morimoto1989}.
...
Where large crystals were present and the mixing model was not required for reduction of laser-ablation data, trace-element compositions are similar among clinopyroxene crystals within individual experiments, indicating that the trace-element concentration of the melt remained spatially homogeneous throughout the experimental charges during the course of the experiments.
% % Fedele: In any case, your trace element data looks definitely more homogeneous (though I think there still is some variability for some elements), which could be considered somewhat strange, given that major element data are not. However, at a more detailed inspection of your Table SM1, it seems that the cpx analysed for trace elements are a selection of the entire dataset, actually featuring limited chemical variability also for major elements. However, I'm not sure this has been ever stated in the manuscript (neither how this selection has been done), giving the impression that analysed minerals are not homogeneous, as evident from figure 4. 
...
\subsection{Attainment of equilibrium}
Experiments used to determine trace-element-partition coefficients must have attained, or at least closely approached, chemical equilibrium. Reversal experiments, where a clinopyroxene re-equilibrates with a melt, are unfortunately not possible owing to sluggish diffusion of most elements through the clinopyroxene structure \citep{VanOrman2001}. As the formation of clinopyroxene necessarily changes the composition of melt, all crystallisation experiments and natural clinopyroxene, strictly speaking only record true equilibrium conditions at their outermost rim. However, where zoning is limited, partition coefficients from non-rim compositions will still closely approach equilibrium.

Several lines of evidence indicate that our experiments preserve conditions close to chemical equilibrium. 
	The experimental charges underwent small degrees of crystallisation ($\le$ 35\%), resulting in small changes to liquid compositions during the run (Fig. \ref{TAS}). Consequently, in our experiments, clinopyroxene display only subtle chemical zonation and cover a small range of major-element compositions within each charge 
...
%%%%%%%%%
Canary Islands clinopyroxene were inspected under reflected light and by SEM. Only euhedral, blade-shaped crystals free of melt inclusions and chemical zonation were selected for analysis, and the corresponding quenched melt was in direct contact (Fig. \ref{3_ChemTransect}). While equilibrium conditions are more challenging to confirm for a natural volcanic system, the euhedral forms, chemical homogeneity of crystals, and congruency between samples from separate eruptions suggest that the crystals grew in a stable environment, and were not subject to chemical or physical perturbations during growth (Fig. \ref{6_D_Spider}).
...
%%%%%%%%%
\subsection{Chemical zonation in the experiment clinopyroxene}
Because trace-elements diffuse slowly through the clinopyroxene structure relative to the melt \citep{VanOrman2001, Zhang2010}, no re-equilibration of trace-elements takes place on an experimental timescale. Only the very rim of an experimentally grown mineral can record equilibrium conditions and the bulk crystal cannot. Experiments designed for the derivation of equilibrium partition coefficients ideally minimise bias by limiting the fraction of crystallisation, producing minerals that are as homogeneous as possible. Currently available \textit{in situ} analytical techniques, such as LA-ICP-MS and SIMS, are limited in terms of minimum beam-size to $\sim$10 $\mu$m; chemical zonation, however subtle, will be continuous from the core to the rim of the mineral. Consequently, no experimentally derived partition coefficients record equilibrium partition coefficients \textit{sensu stricto}, but when properly conducted, will get close.

	The trace-element data collected from our experiments, and from published studies, are average compositions for clinopyroxene that are zoned to varying degrees, the rims of which record equilibrium with the surrounding melt. In situ chemical analyses preferentially sample the mantle and rim of zoned crystals, because few analyses section a crystal perfectly through the core, resulting in a bias toward equilibrium mineral compositions.

	We modelled a worst-case scenario to determine how strongly chemical zoning in clinopyroxene can affect the resultant $D$ values for trace-elements. If 20\% of the melt crystallises as clinopyroxene only, and where the chemical analyses are truly averages of clinopyroxene composition, an incompatible element with a true equilibrium partition coefficient of 0.1 would return a measured partition coefficient of 0.09. For compatible elements with true $D$ values of $\sim 10$, measured partition coefficients can be a factor of 2--3 higher.	
 ...
 In our experiments, the trace-element composition of the melt does not vary as much as in this worst-case scenario model, partially because additional mineral phases have crystallised alongside clinopyroxene. For example, in experiment M3\ce{_2}, the REE are compatible in clinopyroxene, but are incompatible in biotite and oxides \citep[e.g.][]{Mahood1990,Schmidt1999}. The REE have therefore been residually enriched by crystallisation of biotite and oxide minerals, while simultaneously being depleted by crystallisation of clinopyroxene. These two competing processes serve to minimise the effect of fractional crystallisation on the concentration of trace-elements in the melt and consequently their derived partition coefficients.
 This modelling considered, the experimental partition coefficients that we report for incompatible elements should be reliable within analytical uncertainty, whereas the $D$ values for compatible elements should be considered as maxima, but are probably overestimated by no more than 30\%.

	In addition to the Ti analyses discussed above, we have measured Ce concentrations via EPMA point transects across some of our experiment clinopyroxene (Supplement \ref{CeTransects}). The transects reveal clinopyroxene Ce concentrations that follow both sector zoning and concentric growth zoning patterns, as recorded by the $X$Mg of the clinopyroxene. 
    %JS v13: give numbers if possible. "Higher" and "lower" don't mean much by themselves. Quantify as much as you can.
    %CB v13: The concentrations are not quantitative, because interferences could not be corrected for in the EPMA analyses. We've used counts as a proxy.
   Ratios of average Ce counts / rim Ce counts that represent the difference between bulk crystal and rim measurements have a mean of $\sim$1.25, ranging between 1.01 and 1.41, consistent with compatible behaviour of Ce and fractionation of the cores of clinopyroxene crystals away from the chemically active part of the crystallising system. 
   The high values are found for the largest clinopyroxene crystals and in these, because of their size, we were able to avoid the cores in the analyses.
   Ce partition coefficients as reported in our data set are therefore overestimated by a maximum of $\sim$25\%. The small magnitude of this growth zonation effect, relative to the worst case model scenario discussed above suggests that, in the case of incompatible elements, the partition coefficients derived from our experiments will be within analytical uncertainty of the true equilibrium values, whereas for compatible elements, they would be well within an order of magnitude of true values.
	Furthermore, because our laser-ablation data are reduced via a robust regression scheme, elevated concentrations of compatible elements in the cores of concentrically zoned clinopyroxene crystals are not strongly represented in the derived clinopyroxene trace-element concentration of the mix (Fig. \ref{LaserMix}b). LA-ICP-MS partition coefficients that we present are therefore less strongly biased toward high values than the 25\% value suggested by the EPMA data for Ce discussed above.
...
\subsection{Variations in cooling rate and the formation of diffusive boundary layers}
During crystallisation a diffusive boundary layer develops within the melt directly adjacent to the growing minerals. Theoretically, this layer is depleted with respect to compatible elements and enriched with respect to those that are incompatible \citep{Lu1995}. The composition of a boundary layer depends on both the relative enrichment or depletion of elements during the crystallisation process, and the speed at which these elements diffuse through the melt. Rapidly diffusing elements with partition coefficients close to unity will have concentrations closest to that of the bulk melt.

Experiments designed to investigate trace-element-partitioning behaviour might employ slow cooling rates to limit the development of diffusive boundary layers, thus forming crystals from melt that is closer in composition to that of the bulk experiment. Such experiments then run into another problem, in that significant crystallisation may occur at temperatures above that of the final run temperature. Rapidly cooled experiments circumnavigate this issue, but may form diffusive boundary layers during crystal growth that become `flattened out' during the homogenisation stage of the experiment.

Numerous diffusion data have been gathered for silicate melts over the past few decades, and a comprehensive review is given by \citet{Zhang2010}. Diffusion of trace-elements through water-saturated peralkaline melts is rapid, owing to their depolymerised structure. For example La diffusion coefficients are 6 orders of magnitude higher than for granitic compositions of a similar temperature \citep[compare][]{Rapp1986,Behrens2009}. This rapid diffusion serves to minimise the formation of diffusive boundary layers adjacent to growing crystals in our experiments.
Coupled diffusion mechanisms complicate the application measured of single-element diffusion coefficients to a crystallising system \citep{Grove1984,Liang1994,Costa2003}. Here, the diffusive flux of trace-elements may be coupled to gradients in major-element concentration within the melt. 
...
%%NB - this has been lifted straight out of the results section and dropped here. It might need some modification!
...
% Reviewer2: Please discuss how uncertainty is calculated 
 Three markedly different behaviours of rare earth element-partitioning are observed in the experiments and Canary Islands rocks (Fig. \ref{6_D_Spider}). These depend on the aegirine concentration in the clinopyroxene and match the major-element exchange vector domains discussed above. Low-aegirine clinopyroxene (\ce{Ae_{5--25}}), including those from the Canary Islands
 % Fedele: Although they display a similar behaviour, I would say that Canary island partition coefficients are quite different from those obtained for experimental low-aegirine pyroxenes. D values are remarkably lower, both for incompatible and for  compatible elements (see, e.g, the MREE around 3, whereas the low-aegirine cpx have more than 10, up to 50 or so). I think this should be explicitly mentioned in the text.
 , prefer the MREE; medium-aegirine clinopyroxene (\ce{Ae_{25--50}}) show a similar behaviour, save for higher LREE partition coefficients, whereas high-aegirine clinopyroxene (\ce{Ae_{55--95}}) strongly prefer HREE and show incompatible behaviour for the light and middle REE. The REE partition coefficients are 0.3--53, typically 2--6, with minima for LREE and MREE in high-aegirine clinopyroxene (Fig. \ref{6_D_Spider}). Our experimental partition coefficients are about an order of magnitude higher than in most other studies of clinopyroxene-melt element-partitioning that were performed on different bulk compositions (Fig. \ref{7_Al_iv}d,e,f), with the exception of high Si systems, such as the Bandelier Tuff \citep[cf.][]{Olin2010}.
 % Reviewer2: You are mostly comparing with partition coefficients from lower Si systems. How do your results compare with Olin and Wolff?
 %CB v13 check if O+W in database.
  ...
  % Fedele: Exactly, as said in a previous comment. This is why I would not comment these data together with your data on experimental runs.


        \begin{figure}[htp]
        \begin{center}
        \includegraphics[width=0.68\textwidth]{7_D_Al_iv_Literature_Jun2017-01.eps}
        \caption[Selected trace-element-partition coefficients vs. \ce{^{IV}Al}]{Element partition coefficients for HFSE (Ti, Zr, Nb), REE (La, Sm, Yb) and TE (Li, Th) vs. \ce{^{IV}Al}. Literature values (n = 411), including those from the Roman Province, Italy, are from the compilation of \citet{Bedard2014}, with additional, more recent, data from \citet{Mollo2016}. A similar diagram with $X^{M2}_{Na}$ in place of \ce{^{IV}Al} is in Supplement \ref{D_XM2Na}.}
        \label{7_Al_iv}
        \end{center}
        \end{figure}
...
\subsubsection{The clinopyroxene M2 site}		
	...
	Because of systematic covariation of compositional parameters in our small data set of experiments and natural samples (n = 16), a model calibrated with these points alone would be unable to deconvolve the effects of each major-element on partitioning behaviour. We have therefore added published experiments and natural phenocryst-glass pairs (n = 75) to assemble a database covering a wide range of composition, pressure, temperature and oxygen fugacity (data from compilation of \citealt{Bedard2014}, and \citealt{Mollo2016}, 0.0001--3.5 GPa, 650--1345 \dgC, log \fO = IW to MH $\approx$ $\Delta$QFM -5 to +5). clinopyroxene compositions cover $X$Mg 0.031--1, $X_{Na}^{M2}$ 0--0.84 and \ce{Al^T} 0--0.49 c.f.u. and melt compositions vary widely in terms of Mg\# (0--100) and $X$\ce{H2O} (0--0.38).
	 REE partition coefficients also vary significantly (e.g. \ce{$D$_{La}} 0.01--4.79; \ce{$D$_{Sm}} 0.02--47.24, and \ce{$D$_{Yb}} 0.11--8.00). The majority of partition coefficients in the training data set were measured via SIMS or LA-ICP-MS, minimising analytical uncertainty (e.g. from analyses by electron-microprobe).
...
%%%%%%%%%%%%
 ...
 \begin{table}[htpb]
\caption{Summary of urun conditions and run products for the internally-heated pressure vessel experiments.} 
\begin{adjustbox}{max width=\textwidth}
...
\begin{figure}[htpb]
\begin{center}
\includegraphics[width=0.8\textwidth]{S1_TAS_Expglasses_Nov2017.pdf}
